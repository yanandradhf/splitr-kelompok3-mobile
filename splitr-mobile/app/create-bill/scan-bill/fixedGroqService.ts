...
export class FixedGroqService {
  ...
  static async processReceipt(imageUri: string): Promise<OCRResult> {
    ...
    try {
      ...
      console.log(
        "Fixed Groq: Image converted, size: " +
      ...
      if (!response.ok) {
        ...
        console.error(
          "Fixed Groq: API Error: " + response.status + " " + errorText
        );
        throw new Error(
      ...
      console.log(
        "Fixed Groq: Processing completed in " + (Date.now() - startTime) + "ms"
      );
      return result;
    } catch (error) {
...
RETURN HANYA JSON!`;
  ...
  static parseResponse(content: string): OCRResult {
    try {
      ...
      console.error(
        "Fixed Groq: JSON parse failed, trying regex fallback: " + String(error)
      );
  ...
  static processAdvancedOCR(parsed: any, rawText: string): OCRResult {
      ...
      if (item.name) {
        ...
        if (lineTotal > 0 && qty > 1) {
          ...
          if (item.unitPrice && Math.abs(unitPrice - item.unitPrice) > 100) {
            notes.push(
              "Item " + item.name + ": Used lineTotal/qty for unitPrice"
            );
          }
        }
    ...
    const printedSubtotal = parsed.printedSubtotal
      ? Math.round(Number(parsed.printedSubtotal))
      : undefined;
    const printedGrandTotal = parsed.printedGrandTotal
      ? Math.round(Number(parsed.printedGrandTotal))
      : undefined;
    ...
    if (printedSubtotal && printedSubtotal > 0) {
      ...
      if (inferredItems.length === 1 && knownTotal < printedSubtotal) {
        ...
        if (missingAmount > 0) {
          ...
          inferredItems[0].unitPrice = Math.round(
            missingAmount / inferredItems[0].quantity
          );
    ...
    let calculatedTotal =
      itemsSubtotal + orderFee + serviceCharge + tax - totalDiscount;

    // 6. Off-by-one correction
    let finalGrandTotal = calculatedTotal;
    if (
      printedGrandTotal &&
      Math.abs(printedGrandTotal - calculatedTotal) <= 2
    ) {
      const diff = printedGrandTotal - calculatedTotal;
      finalGrandTotal = printedGrandTotal;
      notes.push(
        "Off-by-one correction: " + (diff > 0 ? "+" : "") + diff + " rupiah"
      );
    }
    ...
    if (validItems.length > 0 && itemsSubtotal > 0) {
      if (
        printedGrandTotal &&
        Math.abs(finalGrandTotal - printedGrandTotal) <= 2
      ) {
  ...
  static parseWithRegex(content: string): OCRResult {
    ...
    const patterns = {
      items:
        /(?:(\d+)x?\s+)?([A-Za-z][\w\s]+?)\s+(?:Rp\.?\s*)?(\d{1,3}(?:[.,]\d{3})*|\d+)/gm,
      tax: /(?:pajak|ppn|tax)\s*(?:(\d+)%)?\s*[:\s]*(\d{1,3}(?:[.,]\d{3})*|\d+)/gi,
      serviceCharge:
        /(?:service\s*charge|biaya\s*layanan|layanan)\s*(?:(\d+)%)?\s*[:\s]*(\d{1,3}(?:[.,]\d{3})*|\d+)/gi,
      orderFee:
        /(?:order\s*fee|biaya\s*antar)\s*[:\s]*(\d{1,3}(?:[.,]\d{3})*|\d+)/gi,
    };
